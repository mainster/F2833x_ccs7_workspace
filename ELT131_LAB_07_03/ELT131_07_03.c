...
typedef struct MsgObj {
	Queue_Elem 		elem;		//!< Queue element
	unsigned int 	val;		//!< Value member
} MsgObj, *Msg;						//!< Typedef

/*
 * ===================== main of ELT131_LAB_07_03 =====================
 */
void main(void) {
    InitSysCtrl();
...
void Task1_fxn(void) {
	...
	while(1) {
		...
		Queue_put(GPIO02_Queue, (Queue_Elem *) msgp);
...
void Task2_fxn(void) {
	...
	while(1) {
		...
		msgp = Queue_get(GPIO02_Queue);
		GpioDataRegs.GPADAT.bit.GPIO2 = (msgp->val & 0x01);
	}
}
